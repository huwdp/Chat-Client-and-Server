...
void Leave::setNickname(char* nick)
{
	nickname = new char[strlen(nick)+1];
	strcpy(nickname,nick);
}

char* Leave::getNickname(void)
{
	return nickname;
}

int Leave::encode(char** buffer)
{
...
void Leave::decode(char* buffer, int bufferSize)
{
	int stringSize;
	char* nick;
	//Create the stream
